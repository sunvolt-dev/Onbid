...
export default function TabChecklist({ id }: Props) {
  ...
  return (
    <div className="flex flex-col gap-4">
      {/* 진행률 */}
      <div className="bg-surface shadow-card rounded-xl px-5 py-4">
        <div className="flex items-center justify-between mb-2">
          <p className="text-sm font-semibold text-text-1">점검 진행률</p>
        ...
        {["HIGH", "MID", "LOW"].map((level) => {
          ...
          return (
            <div key={level} className="px-5 py-3">
              <p className="text-[11px] font-semibold text-text-4 mb-2">
              ...
              <div className="flex flex-col gap-2">
                {levelItems.map((item) => (
                  <div key={item.id} className="flex items-center gap-3">
                    ...
                    />
                    <span
                      className={`text-[11px] rounded-full px-2 py-0.5 font-medium ${LEVEL_STYLE[item.level]}`}
                    >
                      {item.level}
                    </span>
                    <span
                      className={`text-sm flex-1 ${
                        item.checked ? "line-through text-text-4" : "text-text-1"
                      }`}
                    >
                      {item.label}
                    </span>
                    {!DEFAULT_ITEMS.find((d) => d.id === item.id) && (
                      <button
                        ...
                        className="text-text-4 hover:text-hot-fg text-xs"
                      >
                        ✕
                      </button>
                    )}
      ...
      {/* 항목 추가 */}
      <div className="bg-surface shadow-card rounded-xl px-5 py-4">
        <p className="text-xs font-semibold text-text-2 mb-3">항목 추가</p>
        <div className="flex gap-2">
          <select
            value={newLevel}
            onChange={(e) => setNewLevel(e.target.value as "HIGH" | "MID" | "LOW")}
            className="text-xs border border-border-strong rounded px-2 py-1.5 bg-surface shrink-0"
          >
          ...
          <input
            ...
            className="flex-1 text-xs border border-border-strong rounded px-3 py-1.5 focus:outline-none focus:border-primary"
          />
          <button
            onClick={addItem}
            className="text-xs bg-primary text-primary-fg px-3 py-1.5 rounded font-medium hover:bg-primary-hover shrink-0"
          >
            추가
          </button>
        </div>
